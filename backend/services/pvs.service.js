...
ORDER BY 
  ...
  getPVsByDossierIdAndStatus: async (id_dossier, status) => {
    const query = `
            SELECT p.*, d.num_bc, c.nom_client 
            FROM pv p
            JOIN dossier d ON p.id_dossier = d.id_dossier
            ...
            ORDER BY p.date_creation DESC
        `;
    const result = await pool.query(query, [id_dossier, status]);
    return result.rows;
  },
...
updatePVvalide: async (id) => {
  ...
  try {
    ...
    const dossierQuery = `
      ...
      RETURNING *
    `;
    const dossierResult = await client.query(dossierQuery, ["finie", dossierId]);
